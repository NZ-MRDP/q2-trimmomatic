"""Tests for Trimmomatic worker actions."""
...
from q2_trimmomatic._trimmomatic import _trim_paired_worker, _trim_single_worker


class WorkerMethodTests(unittest.TestCase):
    """Cover Trimmomatic worker command construction."""
    ...
    def test_trim_single_worker_omits_threads_flag_for_auto(self, mock_run):
        """Automatic thread selection should omit Trimmomatic's thread flag."""
        with tempfile.TemporaryDirectory() as tmpdir:
            sequences = self._make_single_reads_dir(tmpdir)

            _trim_single_worker(sequences=sequences, threads="auto")

        cmd = mock_run.call_args[0][0]
        self.assertNotIn("-threads", cmd)

    @patch("q2_trimmomatic._trimmomatic.subprocess.run")
    def test_trim_paired_worker_includes_threads_flag_for_explicit_value(self, mock_run):
        """Explicit thread counts should be forwarded to Trimmomatic."""
        with tempfile.TemporaryDirectory() as tmpdir:
            paired_sequences = self._make_paired_reads_dir(tmpdir)

            _trim_paired_worker(paired_sequences=paired_sequences, threads=4)
